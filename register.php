    ...
    <div class="reg-body">
        ...
        <form method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" placeholder="Isi dengan Nama Anda" value="<?= htmlspecialchars($_POST['name'] ?? '') ?>" required>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <input type="password" name="password" id="regPassword" class="form-control" placeholder="Minimal 6 karakter" required>
                    <button type="button" class="btn btn-outline-secondary" onclick="togglePass('regPassword','eyeReg1')">
            ...
            <div class="mb-4">
                ...
                <div class="input-group">
                    <input type="password" name="confirm" id="regConfirm" class="form-control" required>
                    <button type="button" class="btn btn-outline-secondary" onclick="togglePass('regConfirm','eyeReg2')">
...
(function () {
    ...
    function cleanWA(val) {
        // Hapus semua kecuali angka
        val = val.replace(/[^0-9]/g, '');
        // Konversi awalan 0 → 62
        if (val.startsWith('0')) val = '628' + val.slice(1);
        // Hapus digit pertama jika bukan 6
        while (val.length > 0 && val[0] !== '6') val = val.slice(1);
        // Hapus digit kedua jika ada dan bukan 2
        // if (val.length > 1 && val[1] !== '2') val = '6' + val.slice(2);
        return val;
    }

    waInput.addEventListener('input', function () {
        this.value = cleanWA(this.value);
    });

    waInput.addEventListener('paste', function (e) {
        e.preventDefault();
        const pasted = (e.clipboardData || window.clipboardData).getData('text');
        this.value = cleanWA(pasted);
    });
})();
