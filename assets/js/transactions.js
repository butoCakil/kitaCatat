...
function openAddModal() {
    ...
    document.getElementById("trxDate").value = localISO;
    setType("expense");
    modal.show();
}
...
function setType(type) {
    ...
    // Filter opsi & optgroup kategori sesuai tipe
    document.querySelectorAll("#trxCategory option").forEach(opt => {
        opt.hidden = opt.dataset.type && opt.dataset.type !== type;
    });
    document.querySelectorAll("#trxCategory optgroup").forEach(grp => {
        grp.hidden = grp.dataset.type && grp.dataset.type !== type;
    });
    // Reset pilihan jika kategori yang sedang dipilih tidak sesuai tipe
    const sel = document.getElementById("trxCategory");
    const chosen = sel.options[sel.selectedIndex];
    if (chosen && chosen.dataset.type && chosen.dataset.type !== type) {
        sel.value = "";
    }
}
