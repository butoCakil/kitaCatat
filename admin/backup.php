...
// ── Aksi: Buka Google Drive ──────────────────────────────────
if (isset($_POST['action']) && $_POST['action'] === 'open_drive') {
    $driveUrl = getAppSetting('drive_folder_url', '');
    if ($driveUrl) {
        header('Location: ' . $driveUrl);
        exit;
    }
}
...
<div class="card mb-4">
    <div class="card-body d-flex align-items-center justify-content-between flex-wrap gap-3">
        ...
        <!-- Tombol backup manual -->
           <?php if (getAppSetting('drive_folder_url', '')): ?>
            <form method="POST">
                <input type="hidden" name="action" value="open_drive">
                <button type="submit" class="btn btn-sm" style="background:#f1f5f9;color:var(--text-secondary);border:none;border-radius:8px;padding:8px 16px;font-size:13px;font-weight:600;display:inline-flex;align-items:center;gap:6px">
